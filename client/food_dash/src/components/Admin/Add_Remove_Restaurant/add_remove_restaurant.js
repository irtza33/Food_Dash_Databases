...
const AdminResAddRem=()=>{
    ...
    const showPending=()=>{
        ...
        axios.post("http://localhost:3001/admin/show_pending",newObj,
    ...
    const removeRes=()=>{
        ...
        axios.post("http://localhost:3001/admin/DeleteR",newObj,
        {headers:{'Content-Type': 'application/json; charset=UTF-8','accessToken':localStorage.getItem("accessToken")}})
        .then(response=>{
            if(response.data.error){
                alert("ERROR")
    ...
    const acceptRes=(id)=>{
        ...
        axios.post("http://localhost:3001/admin/verify_res",newObj,
        {headers:{'Content-Type': 'application/json; charset=UTF-8','accessToken':localStorage.getItem("accessToken")}})
        .then(response=>{
            if(response.data.error){
                console.log(response.data.error)
    ...
    return(
        <div>
            ...
            <br></br>
            {
                activeRestaurants && activeRestaurants.map((d)=>{
                    return(
                        <div key={d.restaurant_id}>
                        <ul><li><h4><b>{d.restaurant_name}</b></h4></li></ul>
                        </div>
                    )
                })
            }
            ...
            {
                pendingRestaurants && pendingRestaurants.map((d)=>{
                    return(
                        <div key={d.restaurant_id}>
                        <ul><li><h4><b>{d.restaurant_name}</b></h4></li></ul>
                        <button onClick={()=>acceptRes(d.restaurant_id)}>Confirm Restaurant</button>
                        </div>
                    )
                })
            }
